...
class AddTodoPopupCardHouses extends StatefulWidget {
  /// {@macro add_todo_popup_card}
  const AddTodoPopupCardHouses({Key? key}) : super(key: key);

  @override
  State<AddTodoPopupCardHouses> createState() => _AddTodoPopupCardHousesState();
}

class _AddTodoPopupCardHousesState extends State<AddTodoPopupCardHouses> {
  ...
  SendHouseData() async {
    ...
    final modelDetailCar = await ItemModelHouse (
      itemPic: imgUrl,
      date: time,
      detail: detail,
    ...
    if (user != null) {
      await FirebaseFirestore.instance.collection('houses').doc(time).set(jsonCar);
    }
